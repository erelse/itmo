package ru.itmo.lessons.lesson19.hw19.task02;
...
public class Task02 {
    public static void main(String[] args) {
        ...
        Map<String, Long> map01 = transactionStream.collect(Collectors.groupingBy(transaction -> transaction.getAccount().getNumber(), Collectors.summingLong(transaction -> transaction.getSum())));
        System.out.println("map01 (номер аккаунта = сумма транзакций по данному аккаунту) " + map01);
        ...
        transactionStream = Stream.of(transaction1, transaction2, transaction3,
                transaction4, transaction5, transaction6, transaction7);
       Long sumAllTransaction = transactionStream.collect(Collectors.summingLong(transaction -> transaction.getSum()));
        System.out.println("Сумма транзакций по всем аккаунтам = " + sumAllTransaction);

       /* второй вариант transactionStream = Stream.of(transaction1, transaction2, transaction3,
                transaction4, transaction5, transaction6, transaction7);
        long sumAllTransaction = transactionStream.map(transaction -> transaction.getSum()).reduce((a,b)->a+b).get();
        System.out.println("Сумма транзакций по всем аккаунтам = " + sumAllTransaction);*/
    }
}
